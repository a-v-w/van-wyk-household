...
- **Working days are a calendar, not a weekday set.** `@/lib/workdays` merges the household default with per-date records whose status is `working`, `off`, `sick` or `leave`. Only `working` counts as a working day, so a sick day silently removes that day's recurring tasks and moves day-before prep. Recurring tasks and meal prep both go through this calendar.
